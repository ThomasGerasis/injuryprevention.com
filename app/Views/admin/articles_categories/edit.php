	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<a href="<?php echo base_url('admin/articleCategories'); ?>" class="breadcrumb-item">Article categories</a>
		...
		<div class="header-elements">
			<div class="breadcrumb justify-content-center">
				<?php if(!empty($page['id'])){?>
					<a href="<?php echo FRONT_SITE_URL.'preview/previewCategory/'.$page['id'].'/'.md5($page['id'].'_SOMALAB_'.$page['permalink']);?>" id="previewChangesBtn" target="_new" class="breadcrumb-elements-item"><i class="icon-file-eye"></i> preview changes</a>
					...
					<?php }else{ ?>
						<a href="<?php echo FRONT_SITE_URL.'preview/previewCategory/'.$page['id'].'/'.md5($page['id'].'_SOMALAB_'.$page['permalink']);?>" target="_new" class="breadcrumb-elements-item" title="Προεπισκόπηση"><i class="icon-file-eye"></i> preview</a>
					<?php } ?>
				<?php } ?>
			</div>
		</div>
...
<div class="content">
	<?php if (!empty($page['id'])) { ?>
		<input id="edit_type" class="d-none" value="articleCategory">
		<input id="edit_type_id" class="d-none" value="<?php echo $page['id']; ?>">
		<input id="edit_back_url" class="d-none" value="<?php echo base_url('articleCategories'); ?>">
	<?php } ?>
	<form method="post" class="myvalidation" id="tinymce_form">
		...
		<div class="row">
			...
			<div class="col-md-4 col-lg-3">
				...
				<div class="form-group">
					<?php $parent_id = (isset($page['parent_id']) ? $page['parent_id'] : ''); ?>
					<select class="form-control" name="parent_id">
						<option value="" <?php echo (empty($parent_id) ? 'selected="selected"' : ''); ?>>-- No parent category --</option>
						<?php foreach($parentCategories as $parentCategory){
							if(!empty($page['id']) && $page['id'] == $parentCategory['id']) continue;?>
				...
				<label>Show article grid on bottom</label>
				<div class="form-group">
					<div class="form-check">
						<label class="form-check-label">
							<input type="checkbox" name="show_article_grid" class="form-check-input" <?php echo (empty($page['show_article_grid']) ? '' : 'checked="checked"');?>>
							Yes
						</label>
					</div>
				</div>
