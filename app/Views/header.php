...
<head>
    ...
    <link rel="stylesheet" href="/dist/css/bootstrap.css?v=2.5<?php echo ENVIRONMENT == 'development' ? time() : '';?>">
    <link rel="stylesheet" href="/dist/css/main.css?v=2.58<?php echo ENVIRONMENT == 'development' ? time() : '';?>">
    <link rel="stylesheet" href="/dist/css/customSwiper.css?v=2.75<?php echo ENVIRONMENT == 'development' ? time() : '';?>">

    <?php if (isset($loadCss) && is_array($loadCss)) : ?>
        <?php foreach ($loadCss as $cssFilename) : ?>
            <link rel="stylesheet" href="<?php echo "/dist/css/$cssFilename.css?v=2.56".(ENVIRONMENT == 'development' ? time() : '');?>">
        <?php endforeach; ?>
    <?php endif; ?>
    <?php if (isset($loadAssetsCss) && is_array($loadAssetsCss)) : ?>
        <?php foreach ($loadAssetsCss as $cssFilename) : ?>
            <link rel="stylesheet" href="<?php echo "/assets/css/$cssFilename.css?v=2.56".(ENVIRONMENT == 'development' ? time() : '');?>">
        <?php endforeach; ?>
    <?php endif; ?>
